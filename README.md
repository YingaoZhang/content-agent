...
用户画像独立存放在 `app/audience_profiles/*.json`，文件名必须与 JSON 中的 `id` 完全一致。当前 8 个画像各自包含角色、目标、痛点、决策因素、证据偏好、异议、语气、内容重点、文章结构、行动号召和视觉方向。启动时会用 `AudienceProfile` 统一校验：缺少必填字段、id 不合规、文件名不一致或字段类型错误都会直接报错，不会带着坏配置生成内容。

后续新增画像只需复制一个 JSON 文件，修改 `id`、`label` 和画像字段即可；`ContentRequest.primary_audience` 使用字符串并对照注册表校验，因此不需要再修改 Python 枚举、提示词或前端 HTML。画像中的未知字段会被保留并自动放入模型上下文，适合逐步增加 `渠道偏好`、`典型问题`、`禁用表达` 等字段。前端从 `/api/audiences` 动态读取画像列表，历史任务仍会保存当时的 `request` 快照。
