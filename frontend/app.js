...
function setBusy(isBusy, detail = "正在读取资料与受众上下文…") {
...
$("#generation-form").addEventListener("submit", async (event) => {
  ...
  const request = { topic: $("#objective").value.trim().slice(0, 120), primary_audience: $("#audience").value, objective: $("#objective").value.trim(), brand_name: $("#brand").value.trim(), call_to_action: $("#cta").value.trim() || "了解更多", image_count: Number($("#image-count").value), theme: $("#theme").value };
  const body = new FormData();
  body.append("request", JSON.stringify(request));
  files.forEach((file) => body.append("files", file));
  setBusy(true, "正在读取资料与受众上下文…");
  try {
    const data = await requestJson("/api/generate", { method: "POST", body });
    setBusy(false);
    updateResult(data);
    showToast("公众号成品已生成");
  } catch (error) {
